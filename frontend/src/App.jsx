...
import { Search, Loader2, Link as LinkIcon, AlertCircle, CheckCircle2, User, BookOpen, GraduationCap, Award, Hash, Clock, Code, Target, Terminal } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background text-text flex flex-col font-sans">
      ...
      <div className="flex-grow z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex flex-col">
        ...
        <header className="mb-12 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">
              Skillrack <span className="text-primary font-mono bg-blue-50 px-2 py-0.5 rounded ml-1">Tracker</span>
            </h1>
          </div>
        </header>
            ...
            <div className="my-auto max-w-3xl mx-auto text-center space-y-8 pb-20">
              <div className="space-y-4">
                ...
                <p className="text-lg md:text-xl text-slate-600 max-w-2xl mx-auto leading-relaxed">
                  Enter your SkillRack profile link and watch as we crunch your coding points, daily challenges, and ranks in seconds.
                </p>
              </div>

              <form onSubmit={handleSubmit} className="mt-10 relative max-w-xl mx-auto">
                <div className="relative group">
                  ...
                  <div className="relative flex items-center bg-white rounded-2xl shadow-xl shadow-blue-900/5 ring-1 ring-slate-100 p-2 border-2 border-transparent focus-within:border-primary transition-colors">
                    ...
                    <input
                      ...
                      required
                      className="flex-1 py-3 px-2 bg-transparent border-none focus:ring-0 text-slate-800 text-lg placeholder-slate-400 outline-none w-full font-mono text-sm"
                    />
                    <button
                      type="submit"
                      disabled={!url}
                ...
                <div className="lg:col-span-1 border border-slate-200 bg-white rounded-3xl p-8 shadow-xl shadow-blue-900/5 relative overflow-hidden group">
                  ...
                  <div className="flex flex-col items-center text-center space-y-4 relative z-10 mb-8 pt-4">
                    ...
                    <div>
                      ...
                      <p className="text-primary font-mono mt-1 font-medium">{data.rollNumber}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-5 text-sm">
                    <div className="flex items-start gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                      ...
                      <div>
                        <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider mb-0.5">Department</p>
                        <p className="font-medium text-slate-800">{data.dept}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                      <div className="p-2 bg-blue-50 text-blue-600 rounded-lg">
                      ...
                      <div>
                        <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider mb-0.5">College</p>
                        <p className="font-medium text-slate-800">{data.college}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                       <div className="p-2 bg-blue-50 text-blue-600 rounded-lg">
                      ...
                      <div>
                        <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider mb-0.5">Year Info</p>
                        <p className="font-medium text-slate-800">{data.yearInfo}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-2 flex flex-col gap-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white/60 backdrop-blur-sm rounded-[2rem] p-8 md:p-10 border hover:border-slate-200 transition-colors border-slate-100/60 shadow-[0_4px_24px_-8px_rgba(0,0,0,0.05)] flex flex-col justify-center">
                      <span className="text-slate-400 text-sm font-bold tracking-widest uppercase mb-4">Solved</span>
                      <div className="flex items-center gap-4">
                        <Award className="w-8 h-8 md:w-10 md:h-10 text-green-500 stroke-[2.5]" />
                        <span className="text-5xl md:text-6xl font-black text-slate-800 tracking-tight">{data.totalSolved?.toLocaleString()}</span>
                      </div>
                    </div>
                    
                    {/* Points Card */}
                    <div className="bg-white/60 backdrop-blur-sm rounded-[2rem] p-8 md:p-10 border hover:border-slate-200 transition-colors border-slate-100/60 shadow-[0_4px_24px_-8px_rgba(0,0,0,0.05)] flex flex-col justify-center">
                      <span className="text-slate-400 text-sm font-bold tracking-widest uppercase mb-4">Points</span>
                      <div className="flex items-center gap-4">
                        <Target className="w-8 h-8 md:w-10 md:h-10 text-indigo-500 stroke-[2.5]" />
                        <span className="text-5xl md:text-6xl font-black text-slate-800 tracking-tight">{data.points?.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Skill Overview */}
                  <div>
                    <h3 className="text-slate-400 font-bold tracking-widest text-sm mb-6 uppercase">Skill Overview</h3>
                    <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-3 gap-5">
                      <SkillBox title="DT" value={data.dt} colorVariant="blue" />
                      <SkillBox title="DC" value={data.dc} colorVariant="teal" />
                      <SkillBox title="Track" value={data.codeTrack} colorVariant="purple" />
                      <SkillBox title="Tutor" value={data.codeTutor} colorVariant="orange" />
                      <SkillBox title="Test" value={data.codeTest} colorVariant="red" />
                    </div>
                  </div>
        ...
        <footer className="mt-8 text-center text-sm font-medium text-slate-400">
           Last updated: {data ? data.lastFetched : new Date().toLocaleTimeString()}
        </footer>
      </div>
    </div>
  );
...
function SkillBox({ title, value, colorVariant }) {
  const colorMap = {
    ...
    purple: 'bg-purple-50/80 text-purple-600 border-purple-100/50 hover:bg-purple-50',
    orange: 'bg-orange-50/80 text-orange-600 border-orange-100/50 hover:bg-orange-50',
    red: 'bg-red-50/80 text-red-600 border-red-100/50 hover:bg-red-50',
  };
  
  return (
    <div className={`rounded-[2rem] p-6 py-8 flex items-center justify-center flex-col gap-3 border transition-all duration-300 hover:scale-[1.03] cursor-pointer shadow-sm ${colorMap[colorVariant]}`}>
